...
public class FeedbackPortletState {

	private static Logger log = Logger.getLogger(FeedbackPortletState.class);
	private static FeedbackPortletUtil feedbackPortletUtil;
	...
	private static FeedbackPortletState portletState = null;
	...
	public static enum FEEDBACK_ACTIONS{
		CREATE_NEW_FEEDBACK
	}
	...
	public static void addError(ActionRequest aReq, String errorMessage,
		...
		try {
			...
			FeedbackPortletState.log.debug("Error including error message", e);
		}
	}
	...
	public static FeedbackPortletState getInstance (PortletRequest request, PortletResponse response ) {
		
		
		Logger.getLogger(FeedbackPortletState.class).info("------getInstance");
		try {
			
			if(new Util().checkmate()==false)
			{
				PortletSession session = request.getPortletSession();
				portletState = (FeedbackPortletState) session.getAttribute(FeedbackPortletState.class.getName(), PortletSession.PORTLET_SCOPE);
				
				if (portletState == null) {
					portletState = new FeedbackPortletState();
					FeedbackPortletUtil util = new FeedbackPortletUtil();
					portletState.setFeedbackPortletUtil(util);
					session.setAttribute(FeedbackPortletState.class.getName(), portletState);
	...
	private void setFeedbackPortletUtil(FeedbackPortletUtil util) {
		// TODO Auto-generated method stub
		this.feedbackPortletUtil = util;
	}


	private static void defaultInit(PortletRequest request, FeedbackPortletState portletState) {
		// TODO Auto-generated method stub
...
//			if (request.getRemoteUser() == null) {
				...
				FeedbackPortletUtil util = FeedbackPortletUtil.getInstance();
				portletState.setFeedbackPortletUtil(util);
				...
				if (request.getRemoteUser() != null
						&& !request.getRemoteUser().equals("")) {
					portletState.getFeedbackPortletUtil().
				...
				portletState.setSendingEmail(portletState.getFeedbackPortletUtil().getSettingsByName("SENDING EMAIL ADDRESS"));
				portletState.setSendingEmailPassword(portletState.getFeedbackPortletUtil().getSettingsByName("SENDING EMAIL PASSWORD"));
				portletState.setSendingEmailPort(portletState.getFeedbackPortletUtil().getSettingsByName("SENDING EMAIL PORT"));
				portletState.setSendingEmailUsername(portletState.getFeedbackPortletUtil().getSettingsByName("SENDING EMAIL USERNAME"));
				portletState.setMobileApplicationName(portletState.getFeedbackPortletUtil().getSettingsByName("MOBILE APPLICATION NAME"));
				portletState.setProxyHost(portletState.getFeedbackPortletUtil().getSettingsByName("PROXY HOST"));
				portletState.setProxyPort(portletState.getFeedbackPortletUtil().getSettingsByName("PROXY PORT"));
				portletState.setApplicationName(portletState.getFeedbackPortletUtil().getSettingsByName("APPLICATION NAME"));
				portletState.setSystemUrl(portletState.getFeedbackPortletUtil().getSettingsByName("SYSTEM URL"));
				
				//Collection<PermissionType> peL = portletState.getFeedbackPortletUtil().getPermissionsByPortalUser(portletState.getPortalUser(), swpService);
...
//			}
	...
	public FeedbackPortletUtil getFeedbackPortletUtil() {
		// TODO Auto-generated method stub
		return this.feedbackPortletUtil;
	}
