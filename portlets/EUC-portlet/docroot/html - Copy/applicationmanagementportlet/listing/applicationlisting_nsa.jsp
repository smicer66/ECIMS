		  ...
		  if(portletState.getApplicationWorkFlowListing()!=null && portletState.getApplicationWorkFlowListing().size()>0)
		  {
			  ...
			  for(Iterator<ApplicationWorkflow> iter1 = portletState.getApplicationWorkFlowListing().iterator(); iter1.hasNext();)
			  {
				  ...
				  }else if(portletState.getCurrentTab()!=null && portletState.getCurrentTab().equals(VIEW_TABS.LIST_FORWARDED_APPLICATIONS_NSA))
				  {
					  ...
					  status = "Forwarded to "+ rel.getName().getValue();
							...
							<div class="btn-group">
								<button type="button" class="btn btn-success">Click Here for Actions</button>
							  	...
							  	<ul class="dropdown-menu" role="menu">
									...
									{
									%>
									<li><a href="javascript: handleButtonAction('addtoexception', '<%=appWf.getId()%>')">Add Applicant to Exception List</a></li>
									<%
									}
									if(app.getApplicant().getExceptionList().equals(Boolean.FALSE))
									{
									...
									{
									%>
									<li><a href="javascript: handleButtonAction('unblacklistapplicant', '<%=appWf.getId()%>')">Remove Applicant from BlackList</a></li>
									<%
									}
									%>
									
									<%
									if(appWf.getApplication().getStatus().equals(ApplicationStatus.APPLICATION_STATUS_APPROVED))
									{
									%>
									<li><a href="javascript: handleButtonAction('addbiometric', '<%=appWf.getId()%>')">Issue Certificate</a></li>
									<%
									}
									%>
								</ul>
