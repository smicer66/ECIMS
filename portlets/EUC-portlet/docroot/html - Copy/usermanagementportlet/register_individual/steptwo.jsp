	...
	<div class="panel  panel-primary">
	  ...
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" method="post" enctype="application/x-www-form-urlencoded">
		  ...
		  <%
		  if(portletState.getUsertype()!=null && 
		  ...
		  {
		  	%>
		    <div style="padding-bottom:5px"> <strong>This User Belongs to This Agency:</strong>
		        <div>
		          	<select class="form-control" name="specagency">
						<option value="-1">-Specify Agency This User Belongs To-</option>
						<%
						
						for(Iterator<Agency> iter = agencyList.iterator(); iter.hasNext();)
						{
							Agency agent = iter.next();
							String agencyName = agent.getAgencyName();
							
							String selected="";
							if(portletState.getAgencyEntity()!=null && portletState.getAgencyEntity().getAgencyName().equalsIgnoreCase(agencyName))
							{
								selected = "selected='selected'";
							}
							%>
							<option <%=selected%> value="<%=agent.getId() %>"><%=agencyName %></option>
							<%
						}
		  ...
		  <div style="padding-bottom:5px; clear:both"><div><strong>Mobile Number:</strong></div>
	        <div>
				<div style="float:left; padding-right:10px">
					<%= portletState.getCountryCode() + portletState.getMobile()%>
			  	</div>
	        </div>
	      </div>
