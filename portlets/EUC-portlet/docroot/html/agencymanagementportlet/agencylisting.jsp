...
<portlet:actionURL var="listing" name="processAction">
	<portlet:param name="action"
		value="<%=AGENCY_ACTIONS.HANDLE_ACTION_ON_LIST_OF_AGENCY.name()%>" />
</portlet:actionURL>

<div style="padding:10px" id="print-content">
<div class="panel panel-info">
  	<!-- Default panel contents -->
  	<div class="panel-heading"><strong>List of Agencies</strong></div>
  	<div class="panel-body">&nbsp;
		...
		<legend>Agency Listing</legend>
		  <%
		  if(portletState.getAgencyListing()!=null && portletState.getAgencyListing().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				...
				<%
			  for(Iterator<Agency> iter1 = portletState.getAgencyListing().iterator(); iter1.hasNext();)
			  {
				 	
				  Agency appWf= iter1.next();
				  %>
				  <tr>
				  	 <td><%=sdf.format(new Date(appWf.getCreatedDate().getTime())) %></td>
					  <td><%=appWf.getAgencyName()!=null ? appWf.getAgencyName() : ""  %></td>
				  	  <td><%=appWf.getAgencyEmail()!=null ? appWf.getAgencyEmail() : ""  %></td>
					  <td><%=appWf.getAgencyPhone()!=null ? appWf.getAgencyPhone() : ""  %></td>
					  <td><%=appWf.getAgencyType()!=null ? appWf.getAgencyType().getValue() : ""  %></td>
					  <td>
					  		<div class="btn-group">
								<button type="button" class="btn btn-success">Click Here for Actions</button>
							  	...
							  	<ul class="dropdown-menu" role="menu">
							  		<li><a href="javascript: handleButtonAction('updateagency', '<%=appWf.getId()%>', '<%=appWf.getId()%>')">Update Agency Detail</a></li>
								</ul>
							</div>
					  </td>
		...
		<input type="hidden" name="selectedAgencyAction" id="selectedAgencyAction" value="" />	
	</form>
</div>
</div>
...
function handleButtonAction(action, usId, aert){
	...
	if(action=='recallcertificate' || action=='canceldisputecertificate')
	{
		...
		{
			document.getElementById('selectedAgencyId').value = usId;
			document.getElementById('selectedAgencyAction').value = action;
