...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=AGENCY_ACTIONS.EDIT_AN_AGENCY.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Create An New Agency</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Agency Details</span></div>
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  ...
			  <div style="padding-bottom:5px"><strong>Agency Type:</strong>
				<div>
				<select class="clear form-control formwidth50" name="agencyType" data-required="true" id="agencyType">
					<option value="-1">-Select One-</option>
					<%
					String selected="";
					for(Iterator<AgencyType> it = agencyTypeList.iterator(); it.hasNext();)
					{
						AgencyType at = it.next();
						selected = "";
						if(portletState.getAgencyEntity()!=null && 
								portletState.getAgencyEntity().getAgencyType().getValue().equalsIgnoreCase(at.getValue()))
						{
							selected = "selected='selected' ";
						}
						%>
						<option <%=selected %> value="<%=at.getValue()%>"><%=at.getValue()%></option>
						<%
					}
					%>
				</select>
			  ...
			  <div style="padding-bottom:5px"> <strong>Agency Name:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getAgencyEntity()==null ? "" : portletState.getAgencyEntity().getAgencyName() %>" name="agencyName" id="agencyName" placeholder="Provide Name of Agency" />
				</div>
			  </div>
			  <div style="padding-bottom:5px"><strong>Contact Name:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getAgencyEntity()==null ? "" : portletState.getAgencyEntity().getContactName() %>" name="contactName" id="contactName" placeholder="Provide Agency's Contact Persons Name" />
				</div>
			  </div>
			  <div style="padding-bottom:5px"><strong>Agency Phone:</strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" value="<%=portletState.getAgencyEntity()==null ? "" : portletState.getAgencyEntity().getAgencyPhone() %>" name="agencyPhone" id="agencyPhone" placeholder="Provide Agency's Phone Number" />
				</div>
			  </div>
			  <div style="padding-bottom:5px"><strong>Agency Email:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getAgencyEntity()==null ? "" : portletState.getAgencyEntity().getAgencyEmail() %>" name="agencyEmail" id="agencyEmail" placeholder="Provide Agency's Email Address" />
				</div>
			  </div>
	      ...
	      <div>
	      	...
	      	<input type="hidden" value="<%=portletState.getAgencyEntity()!=null ? portletState.getAgencyEntity().getId() : "" %>" name="selectedAgencyId" id="selectedAgencyId" />
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('createnewagency')">Edit Agency</button>
