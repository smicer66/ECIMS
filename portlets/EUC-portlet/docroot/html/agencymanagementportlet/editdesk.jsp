...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=AGENCY_ACTIONS.EDIT_AGENCY_DESK.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Edit An Agency Desk</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Edit Agency Desk Details</span></div>
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  <legend>Agency Desk Details
			  </legend>
			  
			  <div style="padding-bottom:5px"><strong>Agency:</strong>
				<div>
				<select class="clear form-control formwidth50" name="agencyType" data-required="true" id="agencyType">
					<option value="-1">-Select One-</option>
					<%
					for(Iterator<Agency> it = portletState.getAgencyListing().iterator(); it.hasNext();)
					{
						Agency at = it.next();
						String selected = "";
						if(portletState.getEndorsementDeskEntity()!=null && portletState.getEndorsementDeskEntity().getAgency().getId().equals(at.getId()))
						{
							selected = "selected='selected'";
						}
						%>
						<option <%=selected %> value="<%=at.getId()%>"><%=at.getAgencyName()%> - <%=at.getAgencyType().getValue() %></option>
						<%
					}
					%>
				</select>
			  ...
			  <div style="padding-bottom:5px"> <strong>Desk Name:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getEndorsementDeskEntity()==null ? "" : portletState.getEndorsementDeskEntity().getEndorseDeskName() %>" name="deskName" id="deskName" placeholder="Provide Name of Agency Desk" />
				</div>
			  </div>
	      ...
	      <div>
	      	...
	      	<input type="hidden" value="<%=portletState.getEndorsementDeskEntity()!=null ? portletState.getEndorsementDeskEntity().getId() : "" %>" name="actId" id="actId" />
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('saveAgencyDesk')">Save Agency Desk</button>
