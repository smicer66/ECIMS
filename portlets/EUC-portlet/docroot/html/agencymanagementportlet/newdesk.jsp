...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=AGENCY_ACTIONS.CREATE_NEW_AGENCY_DESK.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Create An New Agency Desk</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Agency Desk Details</span></div>
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  ...
			  <div style="padding-bottom:5px"><strong>Agency:</strong>
				<div>
				<select class="clear form-control formwidth50" name="agency" data-required="true" id="agency">
					...
					for(Iterator<Agency> it = portletState.getAgencyListing().iterator(); it.hasNext();)
					{
						Agency at = it.next();
						%>
						<option value="<%=at.getId()%>"><%=at.getAgencyName()%> - <%=at.getAgencyType().getValue() %></option>
						<%
					}
					%>
				</select>
			  ...
			  <div style="padding-bottom:5px"> <strong>Desk Name:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getDeskName()==null ? "" : portletState.getDeskName() %>" name="deskName" id="deskName" placeholder="Provide Name of Agency Desk" />
				</div>
			  </div>
	      ...
	      <div>
	      	...
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('createnewagency')">Create New Endorsement Desk</button>
