...
<jsp:include page="/html/applicationattachmentportlet/tabs.jsp" flush="" />

<portlet:actionURL var="listing" name="processAction">
	<portlet:param name="action"
		value="<%=HANDLE_ACTIONS.HANDLE_ACTION_ON_LIST_OF_ATTACHMENT_TYPE.name()%>" />
</portlet:actionURL>

<div style="padding:10px" id="print-content">
<div class="panel panel-info">
  	<!-- Default panel contents -->
  	<div class="panel-heading"><strong>List of Attachment Types</strong></div>
  	<div class="panel-body">&nbsp;
  	</div>
		<form  id="userListFormId" action="<%=listing%>" method="post" enctype="application/x-www-form-urlencoded" style="padding:10px">
			
		<legend>Attachment Types Listing</legend>
		  <%
		  if(portletState.getAttachmentTypesListing()!=null && portletState.getAttachmentTypesListing().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				...
				<%
			  for(Iterator<ApplicationAttachmentType> iter1 = portletState.getAttachmentTypesListing().iterator(); iter1.hasNext();)
			  {
				 	
				  ApplicationAttachmentType ed= iter1.next();
				  
				  String status = "";
				 
					 
					 
				  if(ed.getExpiryDateApplicable()!=null && ed.getExpiryDateApplicable().equals(Boolean.TRUE))
				  ...
				  <tr>
					  ...
					  <td><%=ed.getName()!=null ? ed.getName() : "N/A" %></td>
					  <td><button type="button" class="btn btn-success" 
					  onclick="javascript: handleButtonAction('updateattachtype', '<%=ed.getId()!=null ? ed.getId() : "" %>', '')">Edit Attachment Type</button></td>
			   ...
			   <input type="hidden" name="selectedAttach" id="selectedAttach" value="">
			   <input type="hidden" name="attachAction" id="attachAction" value="">
			  <%
		  }
		  %>
	</form>
...
function handleButtonAction(action, usId, aert){
	...
	if(action=='recallcertificate' || action=='canceldisputecertificate')
	{
		...
		{
			document.getElementById('selectedAttach').value = usId;
			document.getElementById('attachAction').value = action;
