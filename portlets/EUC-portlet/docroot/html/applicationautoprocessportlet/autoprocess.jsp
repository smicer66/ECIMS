...
<div class="panel panel-info">
  	...
  	<div class="panel-heading"><strong>Assign Workflow Positions</strong></div>
  	<div class="panel-body">&nbsp;
  	</div>
		<form  id="userListFormId" action="<%=listing%>" method="post" enctype="application/x-www-form-urlencoded" style="padding:10px">
			
		<legend>Work Flow Positions</legend>
		  <%
		  if(portletState.getAllAgencyListing()!=null && portletState.getAllAgencyListing().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				<thead>
				  <th>Agency</th>
				  <th>Work Flow Process Position</th>
				</thead>
				<%
			  for(Iterator<Agency> iter1 = portletState.getAllAgencyListing().iterator(); iter1.hasNext();)
			  {
				 	
				  Agency agency= iter1.next();
				  WokFlowSetting wfs = null;
				  for(Iterator<WokFlowSetting> iter2 = portletState.getWorkFlowSettingList().iterator(); iter1.hasNext();)
				  {
					  WokFlowSetting wfs1 = iter2.next();
					  if(wfs1.getAgency().getId().equals(agency.getId()))
					  {
						  wfs = wfs1;
					  }
				  }
				  %>
				  <tr>
				  	  <td><%=agency.getAgencyName()!=null ? agency.getAgencyName() : ""  %></td>
					  <td>
					  	<input type="hidden" name="agency" id="agency" value="<%=agency.getId()!=null ? agency.getId() : ""  %>">
					  	<select name="position" id="position"  class="clear form-control formwidth50">
					  		<option value="">Select One</option>
					  		<%
					  		for(int c=0; c<portletState.getAllAgencyListing().size(); c++)
					  		{
					  			selected = "";
					  			if(wfs!=null && wfs.getPositionId().equals(Integer.valueOf(c)))
					  			{
		...
		<button type="submit" class="btn btn-success" style="float:left" onclick="javascript:handleButtonAction('gobackautoprocess', '', '')">Go Back</button>
	    <button type="submit" class="btn btn-success" style="float:right" onclick="javascript:handleButtonAction('gotostepthreeautoprocess', '', '')">Save</button>	
	</form>
</div>
</div>
...
function handleButtonAction(action, usId, aert){
	...
	}else if(action=='gotostepthreeautoprocess')
	{
		if(confirm("Are you sure you want to save this work flow process? Saving it may interrupt the current flow of EUC Applications in the current workflow")))
		{
			document.getElementById('selectedApplications').value = usId;
			document.getElementById('selectedUserAction').value = action;
			document.getElementById('userListFormId').submit();
