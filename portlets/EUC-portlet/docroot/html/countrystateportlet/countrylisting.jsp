...
<portlet:actionURL var="listing" name="processAction">
	<portlet:param name="action"
		value="<%=ACTIONS.MANAGE_COUNTRY.name()%>" />
</portlet:actionURL>
...
<div class="panel panel-info">
  	...
  	<div class="panel-body">&nbsp;
		...
		<legend>Agency Listing</legend>
		  <%
		  if(portletState.getCountryList()!=null && portletState.getCountryList().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				<thead>
				  ...
				  <th>Country</th>
				  <th>Intl Phone Code</th>
				  <th>Action</th>
				
				</thead>
				<%
			  for(Iterator<Country> iter1 = portletState.getCountryList().iterator(); iter1.hasNext();)
			  {
				 	
				  Country appWf= iter1.next();
				  %>
				  <tr>
					  <td><%=appWf.getName()!=null ? appWf.getName() : "N/A"  %></td>
					  <td><%=appWf.getCode()!=null ? appWf.getCode() : "N/A"  %></td>
					  <td>
					  		<div class="btn-group">
								<button type="button" class="btn btn-success">Click Here for Actions</button>
							  	...
							  	<ul class="dropdown-menu" role="menu">
							  		...
							  		<li><a href="javascript: handleButtonAction('editcountry', '<%=appWf.getId()%>')">Edit Country</a></li>
								</ul>
							</div>
					  </td>
