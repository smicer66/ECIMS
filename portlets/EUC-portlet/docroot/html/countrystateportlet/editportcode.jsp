...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=ACTIONS.CREATE_NEW_PORT_CODE.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Update An Importation Port</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Importation Port Details</span></div>
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="multipart/form-data">
	     <fieldset>
			  ...
			  <div style="padding-bottom:5px"> <strong>State:<span style="color:red">*</span></strong>
				<div>
				  <select class="clear form-control formwidth50" name="portstate" id="portstate" data-required="true">
				  	...
				  	for(Iterator<State> it = portletState.getStateList().iterator(); it.hasNext();)
				  	{
				  		State state = it.next();
				  		String selected = "";
				  		if(portletState.getSelectedState()!=null && portletState.getSelectedState().getId().equals(state.getId()))
				  		{
			  ...
			  <div style="padding-bottom:5px"><strong>Port Name:</strong>
				<div>
					<input class="clear form-control formwidth50" type="text" data-required="true" 
					value="<%=portletState.getPortCodeName()==null ? "" : portletState.getPortCodeName() %>" name="portCode" id=""portCode"" 
					placeholder="Provide Port Code" />
				</div>
			  </div>
	      ...
	      <div>
	      	...
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('editportcode', '<%=portletState.getSelectedPortCode()!=null ? portletState.getSelectedPortCode().getId() : ""%>')">Save Port Code</button>
	...
	<div style="padding:top:10px">
		<ul>
			<li>
				...
				<div>Select the state where this port exists belongs to</div>
				<div>&nbsp;</div>
