...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=ACTIONS.CREATE_NEW_STATE.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Update A State</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">State Details</span></div>
	  <div class="panel-body">
	    	
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="multipart/form-data">
	     <fieldset>
			  <legend>State Details
			  </legend>
			  
			  <div style="padding-bottom:5px"> <strong>Select A Country to Add this state to:<span style="color:red">*</span></strong>
				<div>
				  <select class="clear form-control formwidth50" name="statecountry" id="statecountry" data-required="true">
				  	...
				  	for(Iterator<Country> it = portletState.getCountryList().iterator(); it.hasNext();)
				  	{
				  		...
				  		String selected = "";
				  		if(portletState.getSelectedCountry()!=null && portletState.getSelectedCountry().getId().equals(country.getId()))
				  		{
			  ...
			  <div style="padding-bottom:5px"><strong>Name of State:</strong>
				<div>
					<input class="clear form-control formwidth50" type="text" data-required="true" 
					value="<%=portletState.getStateName()==null ? "" : portletState.getStateName() %>" name="statename" id="statename" 
	      ...
	      <div>
	      	...
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('editstatedetails', '<%=portletState.getSelectedState()!=null ? portletState.getSelectedState().getId() : ""%>')">Save State</button>
	...
	<div style="padding:top:10px">
		<ul>
			<li>
				...
				<div>Select the country this state belongs to</div>
				<div>&nbsp;</div>
