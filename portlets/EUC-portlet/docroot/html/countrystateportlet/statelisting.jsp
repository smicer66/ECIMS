...
<portlet:actionURL var="listing" name="processAction">
	<portlet:param name="action"
		value="<%=ACTIONS.MANAGE_STATE.name()%>" />
</portlet:actionURL>

<div style="padding:10px" id="print-content">
<div class="panel panel-info">
  	<!-- Default panel contents -->
  	<div class="panel-heading"><strong>List of States</strong></div>
  	<div class="panel-body">&nbsp;
  	</div>
		<form  id="userListFormId" action="<%=listing%>" method="post" enctype="application/x-www-form-urlencoded" style="padding:10px">
			
		<legend>State Listing</legend>
		  <%
		  if(portletState.getStateList()!=null && portletState.getStateList().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				...
				<%
			  for(Iterator<State> iter1 = portletState.getStateList().iterator(); iter1.hasNext();)
			  {
				 	
				  State appWf= iter1.next();
				  %>
				  <tr>
				  	  <td><%=appWf.getName()!=null ? appWf.getName() : ""  %></td>
					  <td><%=appWf.getCountry()!=null ? appWf.getCountry().getName() : ""  %></td>
					  <td>
					  		<div class="btn-group">
								<button type="button" class="btn btn-success">Click Here for Actions</button>
							  	...
							  	<ul class="dropdown-menu" role="menu">
							  		<li><a href="javascript: handleButtonAction('editstate', '<%=appWf.getId()%>')">Edit This State</a></li>
								</ul>
							</div>
					  </td>
