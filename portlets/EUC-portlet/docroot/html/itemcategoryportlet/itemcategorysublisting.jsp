...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=ITEM_CATEGORY_ACTIONS.HANDLE_ITEM_SUB_CATEGORY_ACTIONS.name()%>" />
</portlet:actionURL>

<div style="padding:10px" id="print-content">
<div class="panel panel-info">
  	<!-- Default panel contents -->
  	<div class="panel-heading"><strong>HS Codes of Importation Items</strong></div>
  	<div class="panel-body">&nbsp;
  	</div>
  	<form  id="startRegFormId" action="<%=proceedToStepThree%>" method="post" enctype="application/x-www-form-urlencoded" >
		<legend>HS Code Listing</legend>
		  <%
		  if(portletState.getItemCategorySubListing()!=null && portletState.getItemCategorySubListing().size()>0)
		  {
			  %>
			  ...
			  <table width="100%" class="table table-hover" id="btable">
				...
				<%
			  for(Iterator<ItemCategorySub> iter1 = portletState.getItemCategorySubListing().iterator(); iter1.hasNext();)
			  {
				  ItemCategorySub ad = iter1.next();
				  
				  %>
				  <tr>
					  <td><%=ad.getName()!=null ? ad.getName() : "" %></td>
					  <td><%=ad.getHsCode()!=null ? ad.getHsCode() : "" %></td>
					  <td><%=ad.getItemCategory()!=null ? ad.getItemCategory().getItemCategoryName() : "" %></td>
					  <td>
					  		<div class="btn-group">
								<button type="button" class="btn btn-success">Click Here for Actions</button>
							  	...
							  	<ul class="dropdown-menu" role="menu">
									<li><a href="javascript: handleButtonAction('edititemsubcategory', '<%=ad.getId()%>')">Edit Item Sub Category</a></li>
								</ul>
							</div>
					  </td>
				  </tr>
