...
Collection<String> applicationAttachmentTypeList = portletState.getItemCategoryPortletUtil().getApplicationAttachmentNames();
Collection<Agency> agencyList = portletState.getAgencyListing();
String selected = "";
%>
...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=ITEM_CATEGORY_ACTIONS.CREATE_NEW_ITEM_CATEGORY.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	...
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Item Category Details</span></div>
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  <legend>Step 1 of 2: Item Category Details
			  </legend>
			  
			  
			  <div style="padding-bottom:5px"> <strong>Name of Item Category:<span style="color:red">*</span></strong>
				<div>
				  <%=portletState.getItemCategoryName()==null ? "" : portletState.getItemCategoryName() %>
				</div>
			  </div>
			  <div style="padding-bottom:5px"><strong>Accompanying Attachment Types for the selected Item category:</strong>
				<div>
				  <select name="attachmenttype" id="attachmenttype" multiple="multiple">
				  		<option value="">-Select At Least One-</option>
				  		<%
				  		if(applicationAttachmentTypeList!=null && applicationAttachmentTypeList.size()>0)
				  		{
				  			for(Iterator<String> it = applicationAttachmentTypeList.iterator(); it.hasNext();)
				  			{
				  				String atc = it.next();
				  				selected = "";
				  				if(portletState.getApplicationAttachmentList()!=null && portletState.getApplicationAttachmentList().contains(atc))
				  				{
				  					selected = "selected='selected'";
				  				}
				  		%>
				  		<option <%=selected%> value="<%=atc%>"><%=atc %></option>
				  		<%
				  			}
				  		}
				  		%>
			  ...
			  <div style="padding-bottom:5px"><strong>Select Agency that can validate the selected attachment type(s) for the selected Item Category:<span style="color:red">*</span></strong>
				<div>
				  <select name="agency" id="agency" multiple="multiple">
				  		...
				  		{
				  			for(Iterator<Agency> it = agencyList.iterator(); it.hasNext();)
				  			{
				  				Agency agency = it.next();
				  				selected = "";
				  				Set<String> lsh = portletState.getSelectedAgency().keySet();
				  				if(portletState.getSelectedAgency()!=null && lsh.contains(agency.getAgencyName()))
				  				{
				  					selected = "selected='selected'";
				  				}
				  		%>
	      ...
	      <div>
	      	...
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('createnewagency')">Next</button>
	      	<div style="clear:both; padding-top:10px; font-size:11px; color:red; font-weight:bold">
	    All fields with red asterisk (*) imply they must be provided<br>
	    To select more than one item, hold the shift key click the select options</div>
