...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Create A New Item Category</h4>
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  ...
			  <div style="padding-bottom:5px"> <strong>Name of Item Category:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getItemCategoryEntity()==null ? "" : portletState.getItemCategoryEntity().getItemCategoryName() %>" name="categoryname" id="categoryname" placeholder="Provide Name of Item Category e.g. Fertilizer, Fireworks" />
				</div>
			  </div>
			  <div style="padding-bottom:5px"><strong>Select Applicant Type(s) That Can Access This Item Category:<span style="color:red">*</span></strong>
				<div>
				  <select name="applicanttype" id="applicanttype" multiple="multiple">
				  		<option value="">-Select At Least One-</option>
				  		<%
				  		if(applicantTypeList!=null && applicantTypeList.size()>0)
				  		{
				  			for(Iterator<ApplicantType> it = applicantTypeList.iterator(); it.hasNext();)
				  			{
				  				ApplicantType appType = it.next();
				  				selected = "";
				  				if(portletState.getApplicantTypeList()!=null && portletState.getApplicantTypeList().contains(appType))
				  				{
				  					selected = "selected='selected'";
				  				}
				  		%>
				  		<option <%=selected %> value="<%=appType.getValue()%>"><%=appType.getValue() %></option>
				  		<%
				  			}
				  		}
				  		%>
	      ...
	      <div>
	      	...
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('createnewagencystepone')">Next</button>
