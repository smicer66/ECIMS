...
<portlet:actionURL var="proceedToStepThree" name="processAction">
	<portlet:param name="action"
		value="<%=PORTCODE_ACTIONS.EDIT_A_PORTCODE.name()%>" />
</portlet:actionURL>
...
<div style="padding-left:10px; padding-right:10px;" class="col-sm-9 panel panel-default" role="news">
	<h4>Edit A New Port Code</h4>
	<div class="panel  panel-primary">
	  <div class="panel-heading"><span style="color:white; font-weight: bold">Port Code Details</span></div>
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" data-validate="parsley" method="post" enctype="application/x-www-form-urlencoded">
	     <fieldset>
			  <legend>Port Code Details
			  </legend>
			  
			 
			  
			  <div style="padding-bottom:5px"> <strong>Port Code:<span style="color:red">*</span></strong>
				<div>
				  <input class="clear form-control formwidth50" type="text" data-required="true" value="<%=portletState.getPortCodeName()==null ? "" : portletState.getPortCodeName() %>" name="portCodeName" id="portCodeName" placeholder="Provide Port Code" />
				</div>
			  </div>
			  
			   <div style="padding-bottom:5px"><strong>State this Port Belongs to:</strong>
				<div>
				<select class="clear form-control formwidth50" name="portCodeState" data-required="true" id="portCodeState">
					...
					for(Iterator<State> it = portletState.getStateList().iterator(); it.hasNext();)
					{
						State at = it.next();
						selected="";
						if(portletState.getPortCodeState()!=null && portletState.getPortCodeState().equals(Long.toString(at.getId())))
						{
							selected = "selected='selected'";
						}
						%>
						<option <%=selected %> value="<%=at.getId()%>"><%=at.getName()%></option>
						<%
					}
					%>
				</select>
	      ...
	      <div>
	      	<input type="hidden" value="<%=portletState.getPortCodeEntity().getId() %>" name="selectedPortCodeId" id="selectedPortCodeId" />
	      	<input type="hidden" value="" name="selectedPortCodeAction" id="selectedPortCodeAction" />
	      	<button type="submit" class="btn btn-success" style="float:right" onclick="javascript:submitForm('savenewportcode')">Save Port Code</button>
...
{
	document.getElementById('selectedPortCodeAction').value=clicked;
