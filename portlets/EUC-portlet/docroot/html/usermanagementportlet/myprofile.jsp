	  ...
	  <div class="panel-body">
	    ...
	    <form  id="startRegFormId" action="<%=proceedToStepThree%>" method="post" enctype="application/x-www-form-urlencoded">
		  	...
		  	<div style="padding-bottom:5px"> <strong>Type Of User:</strong>
					<div>
					  	<%=portletState.getPortalUser()!=null ? portletState.getPortalUser().getRoleType().getName().getValue() : "N/A"%>
					</div>
		  	</div>
		  
		  <%
		  if(portletState.getPortalUser()!=null && portletState.getPortalUser().getAgency()!=null)
		  {
		  	%>
		    <div style="padding-bottom:5px"> <strong>This User Belongs to This Group Of Users:</strong>
		        <div>
		          	<%=portletState.getPortalUser()!=null ? portletState.getPortalUser().getAgency().getAgencyName() : "N/A"%>
					</select>
		        </div>
		      </div>
		  	<%  
		  ...
		  <div style="padding-bottom:5px"> <strong>Surname:</strong>
	        <div>
	          <%=portletState.getPortalUser()==null ? "N/A" : portletState.getPortalUser().getSurname() %>
	        </div>
	      </div>
	      <div style="padding-bottom:5px"><strong>First Name:</strong>
	        <div>
	          <%=portletState.getPortalUser()==null ? "N/A" : portletState.getPortalUser().getFirstName() %>
	        </div>
	      </div>
	      <div style="padding-bottom:5px"><strong>Other Name:</strong>
	        <div>
	          <%=portletState.getPortalUser()==null ? "N/A" : portletState.getPortalUser().getOtherName() %>
	        </div>
	      </div>
	      <div style="padding-bottom:5px"><strong>Email Address:</strong>
	        <div>
	          <%=portletState.getPortalUser()==null ? "N/A" : portletState.getPortalUser().getEmailAddress() %>
	        </div>
	      </div>
	      
	      <div style="padding-bottom:5px"><strong>Date of Birth:</strong>
	        <div>
	          <%=portletState.getPortalUser()==null ? "" : sdf.format(portletState.getPortalUser().getDateOfBirth()) %>
	        </div>
	      </div>
		  <div style="padding-bottom:5px; clear:both"><div><strong>Mobile Number:</strong></div>
	        <div>
				<div style="float:left; padding-right:10px">
					<%=portletState.getPortalUser()!=null ? portletState.getPortalUser().getPhoneNumber() : ""%>
			  	</div>
	        </div>
	      </div>
		  <div style="padding-bottom:5px; clear:both"><div><strong>Nationality:</strong></div>
	        <div>
				<div style="padding-right:10px">
					<%=portletState.getPortalUser()!=null ? portletState.getPortalUser().getState().getCountry().getName() : "N/A"%>
				</div>
	        </div>
	      </div>
		  <div style="padding-bottom:5px; clear:both"><div><strong>State:</strong></div>
	        <div>
				<div style="padding-right:10px">
					<%=portletState.getPortalUser().getState()!=null ? portletState.getPortalUser().getState().getName() : "N/A"%>
				</div>
	        </div>
	      </div>
		  ...
		  <div style="padding-bottom:5px">
			...
			<div>
				<%=portletState.getPortalUser().getTaxIdNo()==null ? "N/A" : portletState.getPortalUser().getTaxIdNo() %>
			</div>
		  </div>
<div>
	      	...
	      	<%
	      	if(portletState.getPortalUser().getId().equals(portletState.getPortalUser().getId()) && 
	      			!portletState.getPortalUser().getRoleType().getName().equals(RoleTypeConstants.ROLE_END_USER))
	      	{
	      	%>
	      	<button type="submit" class="btn btn-success" style="float:right" 
	      	onclick="javascript:handleButtonAction('edituser', '<%=portletState.getPortalUser().getId()%>')"><%=usr1 %></button>
	      	<%
	      	}else
	      	{
