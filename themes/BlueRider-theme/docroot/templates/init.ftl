...
<#assign us = serviceLocator.findService("com.liferay.portal.service.UserLocalService")) />

<#assign userId = getterUtil.getLong($request.get("theme-display").get("user-id"))) />
<#assign ipAdd = us.getUser(userId).getLastLoginIP() />
<#assign ipAddDate = us.getUser(userId).getLastLoginDate() />
...
<#-- ---------- Date ---------- -->
<#-- #set ($displayDate = $dateUtil.getDate($user.getLastLoginDate(), $dateFormat, $locale, $timeZone)) -->
<#assign date = dateUtil />
<#assign current_time = date.newDate() />
<#assign the_year = current_time?date?string("yyyy") />

<#assign user_last_login_date = date.getDate(user.getLastLoginDate(),"dd", locale)/date.getDate(user.getLastLoginDate(),"MM", locale)/date.getDate(user.getLastLoginDate(),"yyyy", locale) />
